...
from tkinter import filedialog, Text, Scrollbar, messagebox,IntVar,scrolledtext, Checkbutton
import PyPDF2
import nltk
...
# Function to find abbreviations in text and their full forms
...
# Function to load the PDF and show the extracted text
def load_pdf():
    global extracted_text, cleaned_text
    ...
    if pdf_file:
        ...
        text_display.delete(1.0, tk.END)  # Clear the text box
        text_display.insert(tk.END, extracted_text)  # Insert the extracted text
        cleaned_text = ""  # Reset cleaned text when a new file is loaded
        messagebox.showinfo("PDF Loaded", "PDF file loaded successfully!")

# Function to display preprocessed text
def display_preprocessed_text():
    global cleaned_text
    ...
    text_display.delete(1.0, tk.END)  # Clear the text box
    text_display.insert(tk.END, cleaned_text)  # Insert preprocessed text

# Function to display the list of abbreviations
def display_abbreviations():
    if not cleaned_text:
        messagebox.showwarning("Warning", "Please preprocess the text first!")
        return
    
    abbr_with_meanings = find_abbreviations(cleaned_text, abbreviations_dict)
    
    text_display.delete(1.0, tk.END)  # Clear the text box
    if abbr_with_meanings:
        for abbr, meaning in abbr_with_meanings.items():
            text_display.insert(tk.END, f"{abbr}: {meaning}\n")
    else:
        text_display.insert(tk.END, "No abbreviations found.")
...
'''
def label_sentences():
    # Run the labeling process in a separate thread
    thread = threading.Thread(target=run_labeling)
    thread.start()

def run_labeling():
    text = cleaned_text.splitlines()  # Assuming cleaned_text is already preprocessed

    # Dictionary to keep track of the most important sentence for each label
    important_sentences = {label: "" for label in label_colors.keys()}
    
    labeled_sentences.delete(1.0, tk.END)  # Clear previous content
    
    for sentence in text:
        if sentence.strip():  # Ignore empty sentences
            predicted_label = predict_label(sentence)  # Model inference
            color = label_colors.get(predicted_label, "black")  # Default color is black if label not found
            
            # Check if the label is selected in the checkboxes
            if checkbox_vars[predicted_label].get() == 1:
                # Update the most important sentence for the label (e.g., longest sentence)
                if len(sentence) > len(important_sentences[predicted_label]):
                    important_sentences[predicted_label] = sentence

                # Insert the labeled sentence in the text box in the main thread
                labeled_sentences.insert(tk.END, f"{sentence}\n", predicted_label)
                labeled_sentences.tag_config(predicted_label, foreground=color)

    # Once all sentences are processed, update the summary in the main thread
    generate_summary(important_sentences)

def generate_summary(important_sentences):
    summary.delete(1.0, tk.END)
    
    for label, sentence in important_sentences.items():
        if sentence:
            color = label_colors.get(label, "black")
            summary.insert(tk.END, f"{label}: {sentence}\n", label)
            summary.tag_config(label, foreground=color)
'''

# Load the summarization model
# Load a model fine-tuned for summarization
summarizer_tokenizer = AutoTokenizer.from_pretrained("facebook/bart-large-cnn")
summarizer_model = AutoModelForSeq2SeqLM.from_pretrained("facebook/bart-large-cnn")

def summarize_text(text):
    inputs = summarizer_tokenizer(text, return_tensors="pt", max_length=1024, truncation=True)
    summary_ids = summarizer_model.generate(inputs["input_ids"], max_length=150, min_length=25, length_penalty=2.0, num_beams=4, early_stopping=True)
    return summarizer_tokenizer.decode(summary_ids[0], skip_special_tokens=True, clean_up_tokenization_spaces=True)

def label_sentences():
    text = cleaned_text.splitlines()
    
    # Collect sentences by label
    sentences_by_label = {label: [] for label in label_colors.keys()}
    ...
    for sentence in text:
        if sentence.strip():  # Ignore empty sentences
            ...
            # Check if the label is selected in the checkboxes
            if checkbox_vars[predicted_label].get() == 1:
                sentences_by_label[predicted_label].append(sentence)
                color = label_colors.get(predicted_label, "black")
                labeled_sentences.insert(tk.END, f"{sentence}\n", predicted_label)
                labeled_sentences.tag_config(predicted_label, foreground=color)

    # Generate summaries for selected labels
    generate_summary(sentences_by_label)



def generate_summary(sentences_by_label):
    summary.delete(1.0, tk.END)
    
    for label, sentences in sentences_by_label.items():
        if sentences:  # Only summarize if there are sentences for the label
            text_to_summarize = " ".join(sentences)
            # Generate summary using the new summarizer
            summary_text = summarize_text(text_to_summarize)
            color = label_colors.get(label, "black")
            summary.insert(tk.END, f"{label}: {summary_text}\n", label)
            summary.tag_config(label, foreground=color)
...
load_button = tk.Button(frame, text="Load PDF", command=load_pdf)
load_button.pack(side=tk.LEFT, padx=5)

# Create a button to show preprocessed text
preprocess_button = tk.Button(frame, text="Preprocess Text", command=display_preprocessed_text)
preprocess_button.pack(side=tk.LEFT, padx=5)

# Create a button to show abbreviations
abbr_button = tk.Button(frame, text="List Abbreviations", command=display_abbreviations)
abbr_button.pack(side=tk.LEFT, padx=5)

# Create a Text widget to display text
text_display = Text(root, wrap=tk.WORD, width=100, height=30)
text_display.pack(pady=10)

# Add a scrollbar to the text display
scrollbar = Scrollbar(root, command=text_display.yview)
scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
text_display.config(yscrollcommand=scrollbar.set)

# Global variables to store text data
extracted_text = ""
cleaned_text = ""
